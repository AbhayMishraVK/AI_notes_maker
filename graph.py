...
def graph_nodes(workflow):
    ...
    workflow.add_node("take_question_and_make_list_node", take_question_and_make_list_node)
    workflow.add_node("get_current_question_node", get_current_question_node)
...
def graph_flow(workflow):
    workflow.add_edge("starting_node", "take_question_and_make_list_node")
    workflow.add_conditional_edges("take_question_and_make_list_node", check_answer_type, {
        'continue': 'get_current_question_node',
        'stop': END
    })
    workflow.add_edge('get_current_question_node', "create_intro_definition_node")
